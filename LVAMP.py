...
#prob = problems.bernoulli_gaussian_trial(kappa=NaN,M=250,N=500,L=1000,pnz=.1,SNR=40) #a Bernoulli-Gaussian x, noisily observed through a random matrix
...
#half
# layers = networks.build_LVAMP(prob,T=6,shrink='half')
#
#
# # plan the learning
...
# sess = train.do_training(training_stages,prob,'LVAMP_half_giid.npz')

#mcp
layers1 = networks.build_LVAMP(prob,T=15,shrink='half')
training_stages1 = train.setup_training(layers1,prob,trinit=1e-4,refinements=(.5, .1, .01))
sess1 = train.do_training(training_stages1,prob,'LVAMP_half_giid.npz')


# x = np.load('LVAMP_scad_giid.npz')
# y = x["log"]
# print(x.files)
# print(y)