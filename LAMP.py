...
prob = problems.random_access_problem(1) # 1 or 2 for compressive random access or massive MIMO
...
layers = networks.build_LAMP(prob,T=10,shrink='mcp',untied=False)

# plan the learning
training_stages = train.setup_training(layers,prob,trinit=1e-3,refinements=(.5,.1,.01) )

# do the learning (takes a while)
sess = train.do_training(training_stages,prob,'LAMP_mcp_rap1_2_2_tied.npz')
