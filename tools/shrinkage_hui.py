...
def shrink_scad_threshold(r,rvar,theta):
    ...
    # split the piecewise linear function into regions
    rgn0 = tf.to_float( tf.abs(r)>a*lam)
    rgn1 = tf.to_float( tf.abs(r)<=a*lam) - tf.to_float( tf.abs(r)<2*lam)
    rgn2 = tf.to_float( tf.abs(r)<=2*lam)-tf.to_float( tf.abs(r)<lam)
    xhat = rgn0*r + rgn1*(((a - 1)*r - tf.sign(r)*a*lam)/(a-2)) + rgn2*(r - tf.sign(r)*lam)
	
    dxdr =  rgn0 + ((a - 1)/(a - 2))*rgn1 + rgn2
    dxdr = tf.reduce_mean(dxdr,0)
    return (xhat,dxdr)


def shrink_mcp_threshold(r, rvar, theta):
    """    MCP threshold function
    """
    a = theta[1]
    lam = theta[0] * tf.sqrt(rvar)
    lam = tf.maximum(lam, 0)
    ...
    # split the piecewise linear function into regions
    rgn0 = tf.to_float(tf.abs(r) > a * lam)
    rgn1 = tf.to_float(tf.abs(r) <= a * lam) - tf.to_float(tf.abs(r) < lam)
    xhat = rgn0 * r + rgn1 * ((r-tf.sign(r)*lam)*(a/(a-1)))
    dxdr = rgn0 + (a/(a - 1)) * rgn1
    dxdr = tf.reduce_mean(dxdr, 0)
    return (xhat, dxdr)

def shrink_half_threshold(r, rvar, theta):
    """    half threshold function
    """
    a = theta[1]
    lam = theta[0] * tf.sqrt(rvar)
    lam = tf.maximum(lam, 0)
    ...
    # v = r.get_shape()
    # len_r = v[0]
    #len_r = v.num_elements()
    # print('###############################')
    # print(v[0])
    #xhat = tf.zeros_like(r)
    pi = tf.constant(math.pi)
    print('###############################')
    print(pi)
    # split the piecewise linear function into regions

    rgn0 = tf.to_float(tf.abs(r) > (3./4) * (lam**(2./3)))
    phi = tf.acos(lam / 8 * ((tf.abs(r) / 3) ** (-1.5)))
    xhat = rgn0 * 4 * r / 3. * (tf.cos(pi / 3 - phi / 3)) ** 2
    # for index in range(len_r):
    #     print('###############################')
    #     print(index)
    #     phi = tf.acos(lam/8*((tf.abs(r[index])/3)**(-1.5)))
    #     xhat[index] = rgn0 * 4*r[index]/3.*(tf.cos(pi/3-phi/3))**2
...
def get_shrinkage_function(name):
	...
	try:
		return {
            ...
            'scad': (shrink_scad_threshold, (1.,3.7)),
            'mcp': (shrink_mcp_threshold, (2., 2.)),
            'half': (shrink_half_threshold, (1., 3.7)),
...
if __name__ == "__main__":
    ...
    -f function : use the named shrinkage function, one of {soft,bg,pwlin,expo,spline,scad}
    """
    try:
